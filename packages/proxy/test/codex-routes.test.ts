...
describe("typed connection information", () => {
  it("publishes Responses, authenticated catalog URL and an available default on port 7133", async () => {
    ...
    expect(info.codex.model_reasoning_summary).toBe("none")
  ...
  it("reports model-specific live verification without confusing browser MCP with native computer tools", async () => {
    ...
    expect(info.codex.models.find(model => model.id === "gpt-6-astra")?.verified_capabilities)
      .toMatchObject({ tool_search: false, mcp_browser: false, hosted_web_search: true, reasoning_summaries: true })
    expect(info.codex.models.find(model => model.id === "gpt-6-astra")?.default_reasoning_summary).toBe("detailed")
    expect(info.codex.models.find(model => model.id === "gpt-5.4-mini")?.default_reasoning_summary).toBe("none")
  ...
  it("publishes detailed as the default summary when Astra is the selected default model", async () => {
    state.models = { object: "list", data: [nativeModel("gpt-6-astra")] }
    const route = createConnectionInfoRoute({ port: 7133, baseUrl: null })
    const info = await (await route.request("/connection-info")).json() as ConnectionInfo
    expect(info.codex.default_model).toBe("gpt-6-astra")
    expect(info.codex.model_reasoning_summary).toBe("detailed")
  })
})
